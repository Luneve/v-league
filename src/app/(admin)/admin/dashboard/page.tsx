...
import { mockOrganizations, mockVolunteers, mockOpportunities, mockAuditLog, mockCurrentSeason } from "@/mocks";
import { SurfaceCard } from "@/components/ui/SurfaceCard";
import { Badge } from "@/components/ui/Badge";
import { formatRelativeTime, formatDate } from "@/lib/utils";

const pendingOrgs = mockOrganizations.filter((o) => !o.verified);

const stats = [
  { label: "Total Users", value: mockVolunteers.length + mockOrganizations.length },
  { label: "Total Orgs", value: mockOrganizations.length },
  { label: "Pending Verification", value: pendingOrgs.length },
  { label: "Active Season", value: mockCurrentSeason.active ? `${formatDate(mockCurrentSeason.startDate)} — ${formatDate(mockCurrentSeason.endDate)}` : "None" },
];
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <SurfaceCard padding="md">
          ...
          <div className="flex flex-col gap-2">
            {mockAuditLog.slice(0, 7).map((entry) => (
