...
interface ProfileClientProps {
  initialProfile: VolunteerProfile | null;
  initialHistory: CompletedEntry[];
}
...
export function ProfileClient({
  ...
  const [vol, setVol] = useState<VolunteerProfile | null>(initialProfile);
      ...
      <SurfaceCard padding="md">
        ...
        {history.length === 0 ? (
          <p className="text-sm text-muted">
            No completed volunteer activities yet.
          </p>
        ) : (
          <div className="flex flex-col gap-3">
            {history.map((entry) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-muted">
                    {entry.organizationName} · {formatDate(entry.date)}
                  </p>
                </div>
  ...
  return (
    <div className="flex items-center justify-between rounded-xl bg-surface-2 p-3">
      ...
      <div className="flex items-center gap-2">
        <p
          className={`text-base font-bold ${danger ? "text-danger" : "text-text-primary"}`}
        >
          {value}
        </p>
        {sub && <div>{sub}</div>}
