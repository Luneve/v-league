...
export async function setApplicationStatus(applicationId: string, newStatus: AppStatus) {
  const supabase = await createClient();
  const { error } = await supabase.rpc("fn_set_application_status", {
...
export async function markCompletion(applicationId: string, result: "completed" | "no_show") {
  const supabase = await createClient();
...
export async function listMyApplications(filters: { status?: AppStatus; page?: number; pageSize?: number } = {}) {
  const supabase = await createClient();
...
export async function listCandidates(opportunityId: string, status?: AppStatus) {
  const supabase = await createClient();
  let query = supabase
    .from("applications")
    .select("*, volunteer_profiles(id, first_name, last_name, nickname, city, bio, league, season_points, lifetime_hours, avatar_url)")
